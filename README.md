...
부모님(사장님)이 폰에서 `floweranbu.co.kr/admin` 에 들어가 사진을 올리면 카탈로그 "보유 종류"에 바로 뜬다. (코드 수정 불필요)
...
- 운영 환불은 `/admin-orders.html`에서 시작해야 결제 outbox와 발주 중지가 먼저 기록됩니다.
  토스 상점관리자 직접 취소는 관리 화면에서 수동 확인을 요구한 주문에만 사용하세요.
